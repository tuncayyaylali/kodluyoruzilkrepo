# Proje 1
##  Problem 1
[22,27,16,2,18,6] -> Insertion Sort
1.  Yukarý verilen dizinin sort türüne göre aþamalarýný yazýnýz.
2 27 16 22 18 6<br>
2 6 16 22 18 27<br>
2 6 16 22 18 27<br>
2 6 16 18 22 27<br>
2 6 16 18 22 27<br>
2.  Big-O gösterimini yazýnýz.
Big-O Gösterimi: O(n^x<sup>2</sup>)
3.  Time Complexity: Average case: Aradýðýmýz sayýnýn ortada olmasý, Worst case: Aradýðýmýz sayýnýn sonda olmasý, Best case: Aradýðýmýz sayýnýn dizinin en baþýnda olmasý.
Average case: O(n^x<sup>2</sup>)
Worst case: O(n^x<sup>2</sup>)
Best case: O(n)
5.  Dizi sýralandýktan sonra 18 sayýsý hangi case kapsamýna girer? Yazýnýz.
Average case kapsamýna girer.
##  Problem 2
[7,3,5,8,2,9,4,15,6] dizisinin Insertion Sort'a göre ilk 4 adýmýný yazýnýz.<br>
2 3 5 8 7 9 4 15 16<br>
2 3 5 8 7 9 4 15 16<br>
2 3 4 8 7 9 5 15 16<br>
2 3 4 5 7 9 8 15 16<br>
